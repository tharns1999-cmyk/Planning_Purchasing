...
export const OverviewPage: React.FC<OverviewPageProps> = ({ referenceDate }) => {
  ...
  const [summary, setSummary] = useState<DashboardSummaryDetail | null>(() => {
    try { return plannerRepository.getDashboardSummary(referenceDate); } catch { return null; }
  });
  const [overviewModel, setOverviewModel] = useState<OverviewMetricsSummary | null>(() => {
    try { return buildOverviewReadModel(plannerRepository.getSnapshot(), referenceDate); } catch { return null; }
  });
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3.5">
        ...
        >
          ...
          <div>
            <div className="text-2xl font-bold text-rose-600 tracking-tight">{overviewModel.shortfallRows.length}</div>
