  ...
  const filteredDeliveryItems = useMemo(() => {
    return allWeekDeliveryItems.filter((item) => {
      ...
      if (searchQuery.trim()) {
        ...
        const matchPo = String(item.poNumber || '').toLowerCase().includes(query);
        const matchCustomer = String(item.customerName || '').toLowerCase().includes(query);
        const matchProduct = String(item.productName || '').toLowerCase().includes(query);
        const matchCode = String(item.productCode || '').toLowerCase().includes(query);
