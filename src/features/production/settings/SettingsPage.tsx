...
export const SettingsPage: React.FC = () => {
  ...
  const handleConfirmReset = (mode: 'CLEAR_OPERATIONAL' | 'RESET_SEED' | 'CLEAR_ALL' = 'RESET_SEED') => {
    if (resetConfirmInput !== 'RESET') {
      return;
    }

    try {
      if (mode === 'CLEAR_OPERATIONAL') {
        plannerRepository.clearOperationalData();
        setNotice('ล้างข้อมูล PO, รายการ FG และ WIP เรียบร้อยแล้ว (คงเหลือข้อมูลสินค้าและลูกค้าไว้)');
      } else if (mode === 'CLEAR_ALL') {
        plannerRepository.clearAllData();
        setNotice('ล้างข้อมูลทั้งหมดให้เป็นค่าว่างเรียบร้อยแล้ว');
      } else {
        ...
        footer={
          <div className="flex flex-wrap items-center gap-2 justify-end">
            ...
            <Button
              ...
              onClick={() => handleConfirmReset('CLEAR_OPERATIONAL')}
            >
              ล้าง PO/แผน/WIP (เก็บสินค้าและลูกค้า)
            </Button>
            <Button
              variant="secondary"
              size="sm"
              disabled={resetConfirmInput !== 'RESET'}
              onClick={() => handleConfirmReset('RESET_SEED')}
            >
              รีเซ็ตเป็น Seed Data
            </Button>
            <Button
              variant="danger"
              size="sm"
              disabled={resetConfirmInput !== 'RESET'}
              onClick={() => handleConfirmReset('CLEAR_ALL')}
            >
              ล้างข้อมูลทั้งหมด (เป็นค่าว่าง)
            </Button>
          </div>
        }
      >
