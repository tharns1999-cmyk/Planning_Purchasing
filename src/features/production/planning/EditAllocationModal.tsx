...
import { AlertCircle, Package, Tag, Building2, FileText, Calendar } from 'lucide-react';
...
export const EditAllocationModal: React.FC<EditAllocationModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (allocation.sourceType === SourceType.FG) {
      if (!Number.isFinite(parsedFgOutputQty) || parsedFgOutputQty <= 0) {
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-3 pt-1">
          ...
          {allocation.sourceType === SourceType.FG && fgDetails && (
            <Input
              label={`ค่าประมาณ FG (${fgDetails.unit}) *`}
              type="number"
              min="1"
              placeholder={`ระบุจำนวน ${fgDetails.unit} ที่คาดว่าจะได้`}
              value={fgOutputQty}
