...
export const PlanningQueuePanel: React.FC<PlanningQueuePanelProps> = ({
  ...
  const filteredFgItems = useMemo(() => {
    ...
    return fgItems.filter((item) => {
      // 0. Hide estimated completed unless toggle is checked
      if (!showCompleted && item.planningStatus === 'ESTIMATED_COMPLETE') {
        return false;
      }
  ...
  }, [fgItems, fgSearchQuery, fgPriorityFilter, fgDueFilter, showCompleted]);
  ...
  const filteredWipItems = useMemo(() => {
    ...
    return baseWipItems.filter((item) => {
      ...
      return (
        ...
        String(item.relatedProduct || '').toLowerCase().includes(q) ||
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200/90 shadow-2xs overflow-hidden flex flex-col h-full">
      ...
      <div className={isMobileOpen ? 'block flex-1' : 'hidden lg:flex lg:flex-col lg:flex-1'}>
        {/* Tab 1: FG Queue */}
      {activeTab === 'FG' && (
        <div className="p-3.5 flex flex-col flex-1 space-y-3 overflow-hidden">
          ...
          <div className="space-y-2.5">
            ...
            <label className="flex items-center gap-1.5 text-xs text-slate-600 font-medium cursor-pointer pt-1 border-t border-slate-100">
              <input
                type="checkbox"
              ...
              />
              <span>แสดงรายการที่วางแผนครบ (ประมาณ)</span>
            </label>
          </div>

          {/* Cards List */}
          <div className="flex-1 overflow-y-auto space-y-2.5 pr-1 max-h-[calc(100vh-250px)] min-h-[380px]">
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <div className="font-bold text-slate-900 text-xs sm:text-sm leading-tight">
                          {item.shortName && item.shortName.trim() ? item.shortName.trim() : item.productName}
                        </div>
                        {item.productCode && (
                      ...
                      <div className="flex flex-col items-end gap-1">
                        ...
                        {item.planningStatus === 'UNPLANNED' && (
                          <Badge variant="slate" label="ยังไม่วางแผน" />
                        )}
                        {item.planningStatus === 'PARTIAL' && (
                          <Badge variant="amber" label="วางแผนบางส่วน" />
                        )}
                        {item.planningStatus === 'ESTIMATED_COMPLETE' && (
                          <Badge variant="emerald" label="วางแผนครบ (ประมาณ)" />
                        )}
                      </div>
                    </div>
                      ...
                      <div className="flex items-center justify-end gap-2 pt-1 border-t border-slate-100">
                        ...
                        {isItemActive ? (
                          <button
                            ...
                            className={
                              "inline-flex items-center gap-1 font-medium text-[11px] px-1.5 py-0.5 rounded transition-colors cursor-pointer " +
