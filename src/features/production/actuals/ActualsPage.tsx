...
export const ActualsPage: React.FC = () => {
  ...
  // Current production week state (defaults to seed data week 2026-07-20)
  const [currentWeek, setCurrentWeek] = useState(() => getProductionWeek('2026-07-20'));
  const [weekData, setWeekData] = useState<ProductionActualWeekDataDetail | null>(() => {
    try { return plannerRepository.getProductionActualWeekData(currentWeek.weekStart); } catch { return null; }
  });
  const [isLoading, setIsLoading] = useState<boolean>(() => !weekData);
            ...
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-slate-700 font-semibold">
                  ...
                  <th className="py-3.5 px-4 text-right">ผลิตได้จริง</th>
              ...
              <tbody className="divide-y divide-slate-200">
                {allocations.map((item) => {
                  ...
                  const unitLabel = isWip ? 'กก.' : 'กล่อง';
                  const producedVal = !isWip && actualSummary.totalBoxQty && actualSummary.totalBoxQty > 0
                    ? actualSummary.totalBoxQty
                  ...
                  return (
                    <tr key={allocation.allocationId} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="py-3.5 px-4 text-right font-bold text-emerald-700 bg-emerald-50/30">
                        {producedVal.toLocaleString()} {unitLabel}
                      </td>
                      <td className="py-3.5 px-4 text-right font-medium text-rose-700 bg-rose-50/30">
                        {actualSummary.totalWasteQty.toLocaleString()} {unitLabel}
                      </td>
                      ...
                      {/* Action Button */}
                      <td className="py-3.5 px-4 text-center">
                        {actualSummary.hasActualRecord || (actualSummary.entriesCount && actualSummary.entriesCount > 0) || actualSummary.totalGoodQty > 0 || actualSummary.totalWasteQty > 0 || (actualSummary.totalBoxQty || 0) > 0 ? (
                          <Button
                            variant="outline"
                            size="sm"
