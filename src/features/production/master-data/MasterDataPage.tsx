...
export const MasterDataPage: React.FC = () => {
  ...
  const [products, setProducts] = useState<ProductMaster[]>(() => {
    try { return plannerRepository.listProducts(true); } catch { return []; }
  });
  ...
  const [customerShortNameInput, setCustomerShortNameInput] = useState('');
  ...
  const [productShortNameInput, setProductShortNameInput] = useState('');
  const [defaultUnitInput, setDefaultUnitInput] = useState('ชิ้น');
  const [estimatedYieldInput, setEstimatedYieldInput] = useState('');
  ...
  const filteredProducts = useMemo(() => {
    return products.filter((prod) => {
      ...
      if (productSearch.trim()) {
        ...
        const matchesCode = String(prod.productCode || '').toLowerCase().includes(query);
        const matchesName = String(prod.productName || '').toLowerCase().includes(query);
        const matchesCust = String(custName || '').toLowerCase().includes(query);
  ...
  const filteredCustomers = useMemo(() => {
    return customers.filter((cust) => {
      if (customerSearch.trim()) {
        ...
        const matchesCode = String(cust.customerCode || '').toLowerCase().includes(query);
        const matchesName = String(cust.customerName || '').toLowerCase().includes(query);
  ...
  const handleOpenEditCustomer = (cust: CustomerMaster) => {
    ...
    setCustomerShortNameInput(cust.shortName || '');
    setCustomerError(null);
    setIsCustomerModalOpen(true);
  };
  ...
  const handleSaveCustomer = async () => {
    ...
    if (editingCustomer) {
      const res = plannerRepository.updateCustomer(editingCustomer.customerId, {
        ...
        shortName: customerShortNameInput,
      });
      if (res.success) {
  ...
  const handleOpenCreateProduct = async () => {
    ...
    setProductCodeInput('');
    setProductNameInput('');
    setProductShortNameInput('');
    setDefaultUnitInput('ชิ้น');
    setEstimatedYieldInput('');
  ...
  const handleOpenEditProduct = (prod: ProductMaster) => {
    ...
    setProductShortNameInput(prod.shortName || '');
    setDefaultUnitInput(prod.defaultUnit);
    setEstimatedYieldInput(prod.estimatedYieldPerBatch ? String(prod.estimatedYieldPerBatch) : '');
    setProductError(null);
    setIsProductModalOpen(true);
  };
  ...
  const handleSaveProduct = async () => {
    ...
    const parsedYield = estimatedYieldInput.trim() ? parseFloat(estimatedYieldInput) : undefined;
    ...
    if (editingProduct) {
      const res = plannerRepository.updateProduct(editingProduct.productId, {
        ...
        shortName: productShortNameInput,
        defaultUnit: defaultUnitInput,
        estimatedYieldPerBatch: parsedYield,
      });
      if (res.success) {
    ...
    <div className="w-full space-y-4 pb-6">
                ...
                <table className="w-full text-xs text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200 text-slate-700 font-bold">
                      ...
                      <th className="py-3 px-4 w-36">ชื่อย่อ (วางแผน)</th>
                ...
                <table className="w-full text-xs text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200 text-slate-700 font-bold">
                      ...
                      <th className="py-3 px-4 w-36">ชื่อเรียกสั้น</th>
                      <th className="py-3 px-4 w-56">ลูกค้า</th>
                      <th className="py-3 px-4 w-24 text-center">หน่วยเริ่มต้น</th>
                      <th className="py-3 px-4 w-28 text-center">Yield/ชุด</th>
                      ...
                      return (
                        <tr key={prod.productId} className="hover:bg-slate-50/60 transition-colors">
                          ...
                          <td className="py-3 px-4 text-center font-semibold text-slate-700">
                            {prod.estimatedYieldPerBatch ? (
                              <span className="text-sky-700 bg-sky-50 px-1.5 py-0.5 rounded border border-sky-200 text-[11px]">
                                ~{prod.estimatedYieldPerBatch} {prod.defaultUnit}
                              </span>
                            ) : (
                              <span className="text-slate-400">-</span>
                            )}
      ...
      >
        <div className="space-y-4">
          ...
          />
          <Input
            label="ชื่อย่อลูกค้า (สำหรับวางแผน / แสดงบนการ์ด)"
            placeholder="เช่น อิ่มอร่อย, CPF, CPALL"
            value={customerShortNameInput}
            onChange={(e) => setCustomerShortNameInput(e.target.value)}
            helperText="ใช้แสดงผลสั้น ๆ บนกระดานวางแผน และการพิมพ์ PDF/PNG เพื่อไม่ให้ข้อความยาวเกินไป"
          />
        </div>
      </Modal>
      ...
      >
        <div className="space-y-4">
          ...
          />
          <Input
            label="ชื่อเรียกสั้น (สำหรับวางแผน)"
            placeholder="เช่น ปังเนย 200g"
            value={productShortNameInput}
            onChange={(e) => setProductShortNameInput(e.target.value)}
            helperText="แสดงบนการ์ดกระดานวางแผนการผลิต"
          />
          <Input
            label="หน่วยเริ่มต้น *"
          ...
          />
          <Input
            label="อัตราผลผลิตต่อชุด (ค่าประมาณ)"
            type="number"
            min="1"
            placeholder="เช่น 6 (กล่อง/ชุด)"
            value={estimatedYieldInput}
            onChange={(e) => setEstimatedYieldInput(e.target.value)}
            helperText="ใช้คำนวณค่าประมาณ FG อัตโนมัติเมื่อวางแผนผลิต"
          />
        </div>
      </Modal>
    </div>
